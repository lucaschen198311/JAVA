...
public class ButtonClick extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		Integer times;
		Integer currTimes = (Integer) session.getAttribute("currTimes");
		if (currTimes == null)
			times = 0;
		else
			times = currTimes+1;
		
		session.setAttribute("currTimes", times);
		request.setAttribute("currTimes", (Integer) session.getAttribute("currTimes"));
